...
The current T-26 workflow (`pr-review.yml`) had 5 steps (eyes reaction, opencode run, post_review.py, manage_reactions.py, remove eyes). Simplified to a single `opencode run` call like T-22 and T-27, where the model handles everything (posting inline comments, reactions, dedup) via `gh api` directly.

- [x] Rewrite `.github/workflows/pr-review.yml` to a single-step workflow matching the T-22/T-27 pattern
- [x] Move all posting and reaction logic into `.agents/skills/pr-review/SKILL.md` as `gh api` commands the model executes
- [x] Remove `.agents/skills/pr-review/scripts/post_review.py`
- [x] Remove `.agents/skills/pr-review/scripts/manage_reactions.py`
- [x] Update dedup approach: instead of pre-computing via Python, have the model query existing unresolved threads with `gh api graphql` and skip duplicates inline
